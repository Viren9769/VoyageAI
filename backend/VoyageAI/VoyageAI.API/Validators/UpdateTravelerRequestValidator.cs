...
{
    /// <summary>
    /// Validator for UpdateTravelerRequest.
    /// 
    /// This validator ensures that when a user updates a traveler, all provided fields
    /// have valid values. Unlike CreateTravelerRequestValidator, all fields are optional
    /// (nullable) since this is a partial update - users can update just the fields they need.
    /// 
    /// Validation Rules (all optional):
    /// 1. FirstName - If provided, 1-100 characters
    /// 2. LastName - If provided, 1-100 characters
    /// 3. MiddleName - If provided, max 100 characters
    /// 4. DateOfBirth - If provided, must not be in the future, max age 120 years
    /// 5. Gender - If provided, max 50 characters
    /// 6. Email - If provided, valid email format
    /// 7. Phone - If provided, max 20 characters
    /// 8. Nationality - If provided, max 100 characters
    /// 9. PassportNumber - If provided, max 50 characters
    /// 10. PassportCountry - If provided, max 100 characters
    /// 11. PassportExpiry - If provided, must not be in the past
    /// 12. EmergencyContactName - If provided, max 200 characters
    /// 13. EmergencyContactPhone - If provided, max 20 characters
    /// 14. Relationship - If provided, max 50 characters
    /// 15. DietaryPreference - If provided, max 500 characters
    /// 16. SpecialRequirements - If provided, max 1000 characters
    /// 17. FrequentFlyerNumber - If provided, max 100 characters
    /// 18. KnownTravelerNumber - If provided, max 100 characters
    ...
    /// </summary>
    public class UpdateTravelerRequestValidator : AbstractValidator<UpdateTravelerRequest>
    {
        /// <summary>
